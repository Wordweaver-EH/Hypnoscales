...
Classification used three ordered groups (low/medium/high) defined by strict
score thresholds at the 33rd/67th percentile boundaries (scores strictly above
the boundary assigned to the higher group). Because the motor composite is a
discrete 0--10 sum, participants tied at the boundary score are assigned to the
lower group, so the realised high group is typically smaller than one third of
the sample (22--32\% depending on dataset); actual proportions are reported
alongside each $\kappa$ estimate. Weighted Cohen's $\kappa$ (linear weights) was used. Both the full-scale total and the overlap-free
...
for high-group classification (strict threshold at the 67th percentile; realised
proportions 22--32\% depending on dataset). Against the full total, PCS reaches
...
\bottomrule
\multicolumn{7}{l}{\parbox{12cm}{\textit{Note.} AUC: high-group on full total (threshold at 67th percentile; realised proportion varies by dataset). Rest score: non-motor items. HGSHS-5:G: \citet{riegel2021hgshs5g, zech2024hgshs5g}.}}\\
\end{tabular}
\end{table}
...
\caption{ROC curves: motor composite classifying high-group on full total (threshold at 67th percentile),
...
motor-pair scores have a low false-positive rate for high-group status; modest
...
\begin{itemize}
  ...
  \item \texttt{a04\_classification.py} --- Weighted Cohen's $\kappa$
    (three threshold groups), AUC, and binary cutoff sweep for sensitivity, specificity,
